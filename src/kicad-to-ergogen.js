...
const resolve_path = function(base_path, new_path) {
  if (path.isAbsolute(new_path)) return new_path
  else return path.join(base_path, new_path)
}

exports.kicad_to_ergogen = function(raw, base_path, logger=()=>{}) {
  let config
  try {
  ...
  config.file = resolve_path(base_path, config.file) 
  
  for (const module of config.modules){
    if (!module.file) {
      module.file = config.file
    } else {
      module.file = resolve_path(base_path, module.file)
    }
    if (!module.out) {
      module.out = path.join(resolve_path(base_path, config.outdir), module.name + ".js")
    } else {
      module.out = resolve_path(base_path, module.out)
    }
  ...
  config.indexfile = resolve_path(base_path, config.indexfile)
...
module.exports = {`
  for (const module of config.modules){
    ...
    const rel_path = path.relative(path.dirname(config.indexfile), module.out)
    index_js += `\n    ${module.name}: require('./${rel_path}'),`
  }
...
exports.footprint = function(footprint_text, nets, logger=()=>{}) {
  let footprint
  try {
    footprint = sexp_parser.parse(footprint_text)
  } catch (err) {
    console.error(`Could not parse the footprint`)
    process.exit(2)
  }
  const silk = []
  for (const [elem_index, elem] of footprint.values.entries()) {
    if (typeof elem == 'object'){
      // change rotation of elem if necessary 
      if (elem.key != 'model') {
        const transform = (at) => {
          out = [output_tranform(at, false, HAS_ROT.includes(elem.key))]
          return out
        }
        elem.change_at('at', true, transform)
      }
    }
    if (elem.key == 'fp_text' && elem.values[0] == 'reference') {
      // add option to hide the reference
      elem.values[1] = '\"${p.ref}\"'
      const hide_index = elem.values.indexOf('hide')
      if (hide_index >= 0) elem.values[hide_index] = '${p.ref_hide}'
      else elem.values.push('${p.ref_hide}')  
    }
    else {
      const layer_index = elem.indexOf('layer')
      if (layer_index >= 0 && elem.values[layer_index].values[0].slice(2) == 'SilkS')
      silk.push(footprint.values.splice(elem_index, 1))
    }
    if (elem.key == 'pad') {
      // add nets if needed
      const pad_number = elem.values[0]
      if (pad_number!='\"\"') {
        if (!nets[pad_number]) nets[pad_number] = {key: `pad${pad_number}`, default: undefined}
        elem.values.push(`\${p.net.${nets[pad_number].key}.str}`)
      }
    }
  }
  footprint.values.splice(3,0,'${p.at /* parametric position */}')
  // bring nets into format {key: default} for file generation
  out_nets = {}
  for (index of Object.keys(nets)) {
    out_nets[ nets[index].key ] = nets[index].default
  }
  // return ergogen file
  out = `// footprint file for ergogen automatically generated by kicad-to-ergogen
// include it in ergogen/src/footprints/index.js using
//     require('path/to/this/file.js')
` + create_footprint_file(out_nets,[footprint])
  return out
}
